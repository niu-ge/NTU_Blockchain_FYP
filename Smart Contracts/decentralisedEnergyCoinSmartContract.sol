...
contract energyCoin is IERC20 {
    ...
    string public customerDataString ="";
    string public ownerDataString ="";
    string public tempString ="";
    string public negaWattString="";
   ...
   constructor() public {
    ...
    function updateMonthlyConsumerDB(string memory dataBlock) public{
        if (msg.sender != energyOwner) { emit accessDeniedTextEvent(accessDeniedText); return; }
        customerDataString = string(abi.encodePacked(customerDataString, dataBlock));
    }
    function updateMonthlyOwnerDB(string memory dataBlock) public{
        customerDataString = string(abi.encodePacked(ownerDataString, dataBlock));
    }
    ...
    function transferFrom(address owner, address buyer, uint256 numTokens) public override returns (bool) {
     ...
     function buyKWH(uint256 noOfWatts, string memory dateTime, string memory wattString, string memory buyerAddress) public returns (bool) {{
        negaWattString = string(abi.encodePacked("-",wattString));
        tempString = string(abi.encodePacked(dateTime,",",buyerAddress,",",negaWattString));
        ...
        customerDataString = string(abi.encodePacked(customerDataString, tempString));
        tempString = "";
        negaWattString="";
        return true;
    }
    ...
    function getCustomerDB() public view returns(string memory){
        return customerDataString;
    }
     function getOwnerDB() public view returns(string memory){
        return ownerDataString;
    }
